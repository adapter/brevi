...
- The desktop app releases separately from npm: `apps/desktop` is private and carries its own version, and `.github/workflows/desktop-release.yml` builds and attaches macOS/Linux artifacts to the GitHub release for a pushed `desktop-v<version>` tag (which must match `apps/desktop/package.json`). Changesets never version it, so a desktop-only change still needs an empty changeset.
