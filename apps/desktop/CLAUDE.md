...
- `updater.ts`: electron-updater wiring; checks a static generic feed 30 seconds after launch and every 6 hours, downloads a new version, and restarts into it as soon as nothing is running or queued (stopping the orchestrator cancels its queue, so a queued run blocks an automatic install exactly like an executing one); `beforeInstall` runs the app's normal quit teardown first so the supervised orchestrator is stopped before the new version starts its own
