...
`run-usage-snapshot` carries a minimized, ccusage-compatible copy of one Claude session's usage (token counts, ids, timestamps, model, and any pre-calculated cost, never prompt or response content), exported after every agent execution. The host validates it (path-safe project key and session id, byte cap `WORKER_MAX_USAGE_SNAPSHOT_BYTES`, optional content hash) and archives it atomically under `~/.brevi/ccusage/claude/projects/`, replacing the session's earlier snapshot so replays never double-count; a snapshot sequenced below a `run-complete` has landed before that completion is acknowledged. A re-export at the end of an attach session travels without a lease, best effort. See [Costs and usage](/guides/costs/) for querying the archive.
