...
- `select.ts`: provider selection (`auto` picks Firecracker only when on Linux, `/dev/kvm` is read-writable, and the binary resolves; otherwise process; `auto` never fails, it downgrades)
