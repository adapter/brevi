...
- `select.ts`: provider selection (`auto` picks Firecracker only when the full preflight passes, the same checks `ensureAvailable()` runs; otherwise process; `auto` never fails, it downgrades)
